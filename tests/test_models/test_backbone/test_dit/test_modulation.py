...
class TestModulateFunction:
    ...
    def test_output_shape(self):
        ...
        shift = torch.randn(4, 64)
        scale = torch.randn(4, 64)
        out = modulate(x, shift, scale)
...
class TestAdaLNModulation:
    ...
    def test_output_count(self):
        ...
        adaln = AdaLNModulation(hidden_dim=64, cond_embed_dim=64)
        c = torch.randn(2, 64)
        outputs = adaln(c)
...
class TestAdaLNBlock:
    ...
    def test_output_shape(self):
        """Test output shape matches input."""
        block = AdaLNBlock(hidden_dim=64)
        x = torch.randn(2, 16, 64)
    ...
    def test_normalization_applied(self):
        ...
        block = AdaLNBlock(hidden_dim=64)
        x = torch.randn(2, 16, 64) * 10 + 5  # Non-normalized input
        shift = torch.zeros(2, 64)
        scale = torch.zeros(2, 64)
        out = block(x, shift, scale)
...
class TestCrossAttentionConditioning:
    ...
    def test_output_shape(self):
        ...
        cross_attn = CrossAttentionConditioning(
            hidden_dim=64, cond_embed_dim=64, num_heads=4
        )
        x = torch.randn(2, 16, 64)
    ...
    def test_output_shape_with_multiple_cond_tokens(self):
        ...
        cross_attn = CrossAttentionConditioning(64, 64, num_heads=4)
        x = torch.randn(2, 16, 64)
        cond = torch.randn(2, 4, 64)  # 4 conditioning tokens
        out = cross_attn(x, cond)
        assert out.shape == x.shape
    ...
    def test_gradient_flow(self):
        ...
        cond = torch.randn(2, 64, requires_grad=True)
        out = cross_attn(x, cond)
        out.sum().backward()
    ...
    def test_different_embed_dims(self):
        ...
        for hidden_dim, cond_dim in [(64, 32), (128, 64)]:
            cross_attn = CrossAttentionConditioning(hidden_dim, cond_dim, num_heads=4)
...
class TestFinalLayerModulation:
    ...
    def test_output_shape(self):
        ...
        final = FinalLayerModulation(
            hidden_dim=64, out_dim=12, cond_embed_dim=64  # 3*2*2 = 12
        )
        x = torch.randn(2, 16, 64)
        c = torch.randn(2, 64)
        out = final(x, c)
    ...
    def test_zero_initialization(self):
        ...
        final = FinalLayerModulation(64, 12, 64)
        c = torch.randn(1, 64)
